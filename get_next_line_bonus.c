...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/03/15 14:16:18 by lolefevr          #+#    #+#             */
/*   Updated: 2023/03/15 14:28:25 by lolefevr         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	char			*str;
	static t_list	*beginlst[1024];

	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	str = NULL;
	ft_read_nl(&beginlst[fd], fd);
	if (!beginlst[fd])
		return (NULL);
	str = ft_extract_str(beginlst[fd]);
	ft_rdy_nl(&beginlst[fd]);
	if (str[0] == '\0')
	{
		ft_reset_lst(beginlst[fd]);
		beginlst[fd] = NULL;
		free(str);
		return (NULL);
	}
